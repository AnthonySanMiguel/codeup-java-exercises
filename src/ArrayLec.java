...
//        for(int temp : temps) { // Hint: Make array names PLURAL so it makes loops easier to write with singulars
...
//        System.out.printf("Updated third temp: %d/n", temps[2]);
        ...
        int randInt = rand.nextInt(horns.length); // Gives us a number between horns.length (index 0-3), 4 is the upper bound or 'limit'
        ...
        final int FORTUNA_TEAM = 3; // Constant types are typically upper-cased

        // Assign a length to an array to hold the instructor team
        Instructor[] team = new Instructor[FORTUNA_TEAM]; // New instructor array object will be length of constant (e.g. declared as 3)
        ...
        // Put those instructor objects in an array
        team[0] = casey; // Hard coded version...can also use a loop for large amounts (see below)
        team[1] = vivian;
        team[2] = trant;
        // Now our 'team' array has three instructor objects inside of it!
        ...
        System.out.println(); // Just print blank line to keep output clean

        // Let's create an array with only the first names of instructors

        String[] firstNames = new String[team.length];
        for(int i = 0; i < team.length; i++){ // Creates array with first names
            firstNames[i] = team[i].getFirstName();
        }
        for(String firstName : firstNames){ // Iterates through first names array and just outputs first names
            System.out.println("\t" + firstName);
        }

        // Overwrite all elements in firstNames array to 'haXX0rz'
        Arrays.fill(firstNames, "haXX0rz"); // Values in first names array should all be replaced with hacker text
        for(String name : firstNames) {
            System.out.println("\t" + name);
        }
//        System.out.println(Arrays.toString(firstNames));
//        System.out.println(Arrays.toString(team)); // Shows target array 'team' with each instructor object's memory location and value

        System.out.println(); // Just print blank line to keep output clean

        // Sum up all instructor frequencies
        int freqSum = 0;
        for(Instructor instructor : team) {
            freqSum += instructor.getBrainWaveFrequency();
        }
        System.out.printf("\tTotal Brainpower: %d MHz\n", freqSum/1000); // 1000 hertz in a megahert
    }
}
